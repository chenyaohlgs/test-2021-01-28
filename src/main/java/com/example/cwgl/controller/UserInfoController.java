...
public class UserInfoController {
    ...
    public String toLogin(HttpServletRequest request, HttpServletResponse response){
        HttpSession session = request.getSession();     //获取客户端识别信息
        if(session.getAttribute(Config.CURRENT_USERNAME)==null){
            return "login";
        }else {
            try {
                response.sendRedirect("/pages/index");      //验证用户通过，重定向到/pages/index界面
            } catch (IOException e) {
                e.printStackTrace();
                return "login";
            }
...
//    @RequestMapping(value = "/login.do",method = RequestMethod.POST)
    ...
    public UserInfo setSessionUserInfo(UserInfo userInfo, HttpSession session){
        List<Privilege> privileges = privilegeService.getPrivilegeByRoleid(userInfo.getRoleid());   //根据Roleid将设定好的权限赋值给该用户
